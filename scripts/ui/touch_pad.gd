## Джойстик — круг, который сам ловит свои касания.
##
## Касание запоминается по номеру пальца: пока этот палец на круге, круг его ведёт, а
## остальные пальцы свободны — ими жмут «Рывок», не отпуская джойстик.
##
## Джойстик аналоговый: плыть можно в любую сторону, и чем дальше палец от середины — тем
## быстрее.
extends Control
...
func _draw() -> void:
	var c := size / 2.0
	var r := size.x / 2.0
	draw_circle(c, r, Color(0.07, 0.08, 0.1, 0.5) if floating else Color(0.09, 0.15, 0.19, 0.8))
