...
func test_магазин(c) -> void:
	var e := Evolution.create()
	c.ok("без ДНК не купить", not e.buy("heat").ok)
	e.add_dna(100.0)
	var free := e.dna_free()
	c.ok("термозащита куплена", e.buy("heat").ok)
	c.eq("ДНК ушла", e.dna_free(), free - 30)
	c.ok("второй раз — нечего", e.buy("heat").message.contains("полностью"))
	e.add_dna(200.0)
	var slots := e.slots()
	e.buy("room")
	...
	var old := Evolution.from_dict({"dna_total": 60, "unlocked": {"filter": 1, "cilia": 1, "thermo": 2}, "body": [{"id": "filter", "a": 0}, {"id": "thermo", "a": 180}]})
	c.eq("старая термооболочка стала покупкой", old.upgrade_level("heat"), 1)
	c.eq("и даром", old.shop_spent(), 0)
	c.ok("а с тела она ушла", not old.body.any(func(p): return p.id == "thermo"))
	var p := Creature.of_player(e)
	c.ok("клетка не боится жары", p.heatproof and not p.coldproof)
